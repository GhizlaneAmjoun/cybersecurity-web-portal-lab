...
- _Application:_ Static website (“The Cat Corner”)
- _Languages:_ HTML, JavaScript
- _Code Editor:_ Visual Studio Code
- _Version Control:_ Git, GitHub
- _Local Hosting:_ Python HTTP server (python3 -m http.server 8080)
- _Accessed via:_ http://127.0.0.1:8080
  
**Site Structure/Flow**

- Users start at the home page: _index.html_
- Navigation is provided to a login page: _login.html_
- Successful login redirects to a “success” page: _success.html_
- Hidden page exists outside the normal navigation flow: _secret.html_
...
Through this testing, I observed several common HTTP status codes such as:
...
- Implementing authentication and authorization controls for restricted resources
- Reducing unnecessary server information exposed in HTTP headers
- Regularly reviewing HTTP responses and headers for information disclosure
...
- Removing sensitive comments and internal notes before deployment
- Avoiding exposing references to hidden or internal resources in frontend code
- Implementing authentication and authorization controls for restricted content
- Enforcing access restrictions on the server side rather than relying on hidden URLs
...
- Moving authentication logic to the backend
- Storing passwords securely using hashing
- Removing hardcoded credentials from frontend code
- Avoiding weak or commonly used passwords
- Implementing protections against repeated login attempts
- Considering multi-factor authentication for higher-risk systems
...
- Blocking access to .git/ and other version control directories at the server level
- Ensuring development and configuration files are not deployed to public directories
- Regularly testing applications for exposed files and directories
- Removing unnecessary system, metadata, and repository files before deployment
...
- _CWE-602 (Client-Side Enforcement of Server-Side Security):_ Authentication logic was implemented entirely on the client side
- *CWE-798 (Use of Hard-coded Credentials)*: Credentials were embedded directly in frontend source code and visible to users
- *CWE-1391 (Use of Weak Credentials):* A weak and common password was used
...
- *CWE-538 (Insertion of Sensitive Information into Externally-Accessible File or Directory):* Hidden resources were discoverable through automated enumeration techniques
...
Overall, the application showed how quickly security gaps can appear in even a small, static web system when access control and secure design are not considered from the start. Across different parts of the testing, the main issue wasn’t a single vulnerability, but the combination of exposed resources, client-side logic, and missing protective controls that made internal information easily discoverable. From a CIA triad perspective, confidentiality is at risk due to exposed source code, developer comments, hardcoded credentials, and publicly accessible repository files. An attacker could use these weaknesses to rebuild the application structure and pull sensitive information directly from the codebase without needing any authentication. Integrity is also at risk because authentication is fully client-side and can be bypassed or manipulated. This weakness, especially combined with exposed credentials and weak password practices, allows an attacker to gain unauthorized access and potentially change application behaviour or data. Due to the lack of account lockout or rate limiting features, availability is also at risk. An attacker could run repeated automated login attempts or excessive requests and overwhelm the system, potentially blocking legitimate users from accessing the application.

## Unexpected Challenges

During the project, I ran into a few unexpected issues which led to practical lessons along the way.

- *Login script bug*: While building the login page, I included the event parameter in the loginCheck function without fully understanding its function. During testing, the form wasn’t submitting correctly when clicking the login button. After digging into it, I learned about event.preventDefault(), which stops the default form behaviour so the page doesn’t refresh before the login logic runs. Once added, the login flow worked as expected.
- *Committing changes*: At one point, the website stopped working so I went through my commit history to try to find the issue. Unfortunately, I had grouped multiple changes into a single commit, which made it hard to find the issue. Eventually, I realized the issue wasn’t in the code itself but related to moving the project folder. Still, it highlighted how important smaller, incremental commits are for tracking changes and isolating problems faster.
- *Error: Connection Reset by Peer*: While running Gobuster with a SecLists wordlist, some requests returned a “connection reset by peer” error while others worked normally. I wanted to understand why so I investigated and found out this usually happens when the server closes the connection unexpectedly. I learned that the local Python server isn’t designed to handle aggressive or unusual automated requests so this can lead to unstable responses during enumeration.

## Key Takeaways

Overall, this project helped me connect theory with practical hands-on experience across web development and basic cybersecurity analysis. To summarize:
...
- I improved my understanding of CWE’s, OWASP Top 10 concepts, and the CIA triad and how they apply in real applications
